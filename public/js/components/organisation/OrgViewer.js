    ...
    setup( props, context ) {
        ...
        const fetchOrg = async (oe) => {
            try {
              ...
              const url = `${full}/extensions/FHC-Core-Personalverwaltung/apis/v1/OrgAPI/getOrgStructure?oe=${oe}`;
        ...
        const fetchPersons = async (oe) => {
            try {
              ...
              let full = FHC_JS_DATA_STORAGE_OBJECT.app_root + FHC_JS_DATA_STORAGE_OBJECT.ci_router;                 
              const url = `${full}/extensions/FHC-Core-Personalverwaltung/apis/v1/OrgAPI/getOrgPersonen?oe=${oe}`;        
              const res = await fetch(url)
              let response = await res.json()    
              return { response };
            } catch (error) {
